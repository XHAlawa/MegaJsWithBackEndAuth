...
class Storage extends EventEmitter {
  ...
  login (originalCb) {
    ...
    const handleFinishLoginResponse = (response, aes) => {
    ...
    const finishLogin = (request, aes) => {
      ...
      this.api.request(request, (err, response) => {
        if (err) return cb(err)
        handleFinishLoginResponse(response, aes);
      })
    }
    if (this.options.backendLogin != null) {
      console.log("Login To Mega Using Back End")
      this.options.backendLogin(this.api.counterId).then(result => {
        console.log("Login To Mega Using Back End, Response Recevied")
        ...
        parsedResponse = Array.isArray(parsedResponse) ? parsedResponse[0] : parsedResponse;
        let uint8Array = base64ToUint8Array(result.aesBase64)
        handleFinishLoginResponse(parsedResponse, new AES(Buffer.from(uint8Array)));
      });
    } else {
